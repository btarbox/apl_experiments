    ...
    def handle(self, handler_input):
       ...
       # logger.info("at ButtonEventHandler.handle " + str(handler_input.request_envelope.request.source))
        ...
        if first_arg == 'radioButtonText':
            #the_text = handler_input.request_envelope.request.arguments[1].get("radioButtonText", "not found")
            #handler_input.response_builder.speak(f"Ok, we'll {the_text}").ask("Radio button selected")
            ...
            for button in buttons:
                value = "red" if handler_input.request_envelope.request.arguments[1]['radioButtonId'] == button else "green"
                ...
                button_commands.append(set_value_command)
            logger.info("The command  is: " + str(button_commands))
            return (handler_input.response_builder.speak(f"picked {radio_button_id} button").add_directive(ExecuteCommandsDirective(token=TOKEN,commands=button_commands)).response)

            # temp_data_source['radioButtonExampleData']['radioButtonGroupItems'][0]['radioButtonChecked'] = "False"
            # temp_data_source['radioButtonExampleData']['radioButtonGroupItems'][1]['radioButtonChecked'] = "True"
            # temp_data_source['radioButtonExampleData']['radioButtonGroupItems'][2]['radioButtonChecked'] = "False"
            # temp_data_source['radioButtonExampleData']['defaultSelectedOption'] = "Results"
            # return (
            #     handler_input.response_builder
        ...
        if first_arg == 'radio button':
            ...
            handler_input.attributes_manager.session_attributes = session_attr
            handler_input.response_builder.speak(f"Ok, we'll tell you about team {session_attr['radioButtonText']}").ask("Radio button selected")
